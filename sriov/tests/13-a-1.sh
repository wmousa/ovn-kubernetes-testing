...
# Host network Pod to Service via NodePort, service endpoint serves on Node IP (Host-Network) Endpoint on same host(Node IP used to access is the node pod is scheduled to
source ../config
