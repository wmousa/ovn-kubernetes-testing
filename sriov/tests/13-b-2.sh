...
# Host network Pod to Service via NodePort, service endpoint serves on Node IP (Host-Network) Endpoint on different host(Node IP used to access is a different Node IP
source ../config
