...
then
    # Delete ovnkube-node
    echo "delete ovnkube-node and kube-sriov-device-plugin"
    ...
    kubectl get pods -n ovn-kubernetes -o wide | grep kube-sriov-device-plugin | grep -E "$worker1|$worker2" | awk '{print $1}' | xargs kubectl delete pod -n kube-system
